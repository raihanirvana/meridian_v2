...
Screening works the same way: if `SCREENING_API_BASE_URL` is empty, `runLive.ts`
uses Meteora Pool Discovery directly.
...
```dotenv
METEORA_POOL_DISCOVERY_BASE_URL=https://pool-discovery-api.datapi.meteora.ag
...
LLM_BASE_URL=https://...
LLM_API_KEY=...
SCREENING_API_KEY= # only needed for a custom SCREENING_API_BASE_URL
ANALYTICS_API_KEY=...
JUPITER_API_KEY=...
TELEGRAM_BOT_TOKEN=...
```
