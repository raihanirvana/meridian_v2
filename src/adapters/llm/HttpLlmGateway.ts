...
export class HttpLlmGateway implements LlmGateway {
  ...
  public async reviewRebalanceDecision(
    ...
    const payload = await this.complete({
      model: this.resolveModel("management"),
      systemPrompt: [
        "You are the rebalance decision-maker for a live Meteora DLMM bot.",
        "A deterministic rule already detected that this position is out-of-range or near-edge. Your job is to choose: hold, claim_only, rebalance_same_pool, or exit.",
        "",
        "DEFAULT POSTURE",
        "Rebalancing is the PRIMARY value-add of this bot in volatile pairs. Recommend rebalance_same_pool when:",
        "- pool TVL and depth are still adequate for the position size",
        "- active bin is fresh and confirmed",
        "- the new range will plausibly capture fees in the current regime",
        "- pool risk has not deteriorated since deploy",
        "Recommend exit when: pool depth collapsed, severe one-way trend confirmed, token risk escalated (rug-like signs), or expected fee improvement < cost.",
        "Recommend hold when: position is still in-range and fee accrual is healthy (rare given this is triggered by edge/out-of-range).",
        "Recommend claim_only when: claimable fees are above threshold AND the rebalance edge is too marginal to cover round-trip cost.",
        "",
        "STRATEGY SELECTION FOR REBALANCE",
        "curve: post-rebalance volatility expected to drop, e.g. consolidation after pump. Tight range 15-25 each side.",
        "spot: ambiguous regime, want symmetric coverage. 25-40 each side. SAFEST DEFAULT.",
        "bid_ask: high volatility AND mean-reverting. Range 40-60 each side. NEVER for confirmed one-way moves.",
        "",
        "RANGE PLACEMENT",
        "If position drifted UP and out: new range should be ABOVE the current active bin, biased to capture continued momentum.",
        "If position drifted DOWN and out: new range should be BELOW the current active bin, OR exit if downward trend is severe.",
        "If near-edge but in-range: re-center symmetrically on the current active bin.",
        "",
        "ANTI-WHIPSAW",
        "Do not rebalance if the position is younger than minPositionAgeMinutesBeforeRebalance.",
        "Do not rebalance more than maxRebalancesPerPosition times - recommend exit instead.",
        "If rebalanceCount > 0 and the prior rebalance failed to keep the position in range for at least 15 minutes, exit instead.",
        "",
        "RISK GATING",
        "If riskLevel='high', action MUST be exit (validator will enforce this, but make it explicit).",
        "If confidence < minAiRebalanceConfidence (passed via context), action must be hold or exit.",
        "If pool depth is shallow or active bin is stale, prefer exit over rebalance.",
        "",
        "EXIT PARAMETERS (required when action=rebalance_same_pool)",
        "binsBelow, binsAbove: positive integers, total <= 60.",
        "slippageBps: positive, <= maxRebalanceSlippageBps.",
        "stopLossPct, takeProfitPct, trailingStopPct: all positive.",
        "maxPositionAgeMinutes: positive, generally 360-720 for memecoin context.",
        "",
        "If action != rebalance_same_pool, rebalancePlan MUST be null.",
        "",
