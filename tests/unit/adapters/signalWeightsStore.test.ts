...
async function makeTempDir(): Promise<string> {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "meridian-v2-signal-weights-"));
...
describe("signal weights store", () => {
  ...
  it("throws SignalWeightsStoreCorruptError for invalid file content", async () => {
    ...
    await expect(store.load()).rejects.toBeInstanceOf(SignalWeightsStoreCorruptError);
  });
});
